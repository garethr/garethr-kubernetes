
# This file is automatically generated by puppet-swagger-generator and
# any manual changes are likely to be clobbered when the files
# are regenerated.

require_relative '../../../puppet_x/puppetlabs/kubernetes/provider'

Puppet::Type.type(:kubernetes_service_port).provide(:swagger, :parent => PuppetX::Puppetlabs::Kubernetes::Provider) do

  mk_resource_methods

  def self.instance_to_hash(instance)
    {
    ensure: :present,
    name: instance.metadata.name,
    
      
        name: instance.name.respond_to?(:to_hash) ? instance.name.to_hash : instance.name,
      
    
      
        protocol: instance.protocol.respond_to?(:to_hash) ? instance.protocol.to_hash : instance.protocol,
      
    
      
        port: instance.port.respond_to?(:to_hash) ? instance.port.to_hash : instance.port,
      
    
      
        target_port: instance.targetPort.respond_to?(:to_hash) ? instance.targetPort.to_hash : instance.targetPort,
      
    
      
        node_port: instance.nodePort.respond_to?(:to_hash) ? instance.nodePort.to_hash : instance.nodePort,
      
    
    object: instance,
    }
  end

  def create
    Puppet.info("Creating kubernetes_service_port #{name}")
    create_instance_of('service_port', name, build_params)
  end

  def flush
   unless @property_hash.empty?
     unless resource[:ensure] == :absent
        flush_instance_of('service_port', name, @property_hash[:object], build_params)
      end
    end
  end

  def destroy
    Puppet.info("Deleting kubernetes_service_port #{name}")
    destroy_instance_of('service_port', name)
    @property_hash[:ensure] = :absent
  end

  private
  def self.list_instances
    list_instances_of('service_port')
  end

  def build_params
    params = {
    
      
        name: resource[:name],
      
    
      
        protocol: resource[:protocol],
      
    
      
        port: resource[:port],
      
    
      
        targetPort: resource[:target_port],
      
    
      
        nodePort: resource[:node_port],
      
    
    }
    params.delete_if { |key, value| value.nil? }
    params
  end
end
